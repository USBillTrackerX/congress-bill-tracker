...
def fetch_upcoming_markups(days_ahead: int = 14) -> list:
    ...
    now = datetime.now()
    # Use start of today to include today's markups
    today_start = now.replace(hour=0, minute=0, second=0, microsecond=0)
    cutoff = now + timedelta(days=days_ahead)
    
    all_markups = []
    markups_found = 0
    markups_filtered_past = 0
    markups_filtered_future = 0
    ...
    for chamber in ['house', 'senate']:
        ...
        for meeting_item in meetings_list:
            ...
            # Only include markups from today onwards, within window
            if meeting_date < today_start:
                markups_filtered_past += 1
                continue
            if meeting_date > cutoff:
                markups_filtered_future += 1
                continue
    ...
    all_markups.sort(key=lambda x: x.get('parsed_date', datetime.max))
    
    logger.info(f"Found {markups_found} total markups, {markups_filtered_past} in past, "
                f"{markups_filtered_future} too far ahead, {len(all_markups)} in window")
    return all_markups
...
def find_scheduled_event_for_bill(bill_type: str, bill_number: int, days_ahead: int = 14) -> Optional[dict]:
    """
    Check if a bill has any scheduled events (markup or hearing) in the next X days.
    ...
    # Create normalized bill ID for matching
    bill_id_normalized = normalize_bill_id(bill_type, bill_number)
    
    # Also create formatted versions for matching in titles
    bill_id_formatted = format_bill_type(bill_type) + " " + str(bill_number)  # e.g., "H.R. 471"
    
    events_found = []
    
    for chamber in ['house', 'senate']:
        meetings_list = fetch_committee_meeting_list(chamber, limit=50)
        ...
        for meeting_item in meetings_list:
            ...
            meeting_type = details.get('type', 'Meeting')
            
            # Check if this bill is mentioned in the title
            if bill_id_formatted not in title:
                continue
            
            # Check date is in window
            meeting_date = parse_meeting_date(details.get('date', ''))
            if not meeting_date:
                continue
            if meeting_date < today_start or meeting_date > cutoff:
                continue
            
            # Check status is not canceled
            status = details.get('meetingStatus', 'Scheduled')
            if status == 'Canceled':
                continue
            
            # Determine event type
            if is_markup_meeting(details):
                event_type = 'Mark-up'
            else:
                event_type = 'Hearing'
    ...
    events_found.sort(key=lambda x: x['date'])
