...
def is_significant_action(action_text: str, bill_type: str = '') -> bool:
    """
    ...
    - House bills "placed on calendar" (routine procedure, low signal)
    """
    ...
    if any(keyword in action_lower for keyword in significant_keywords):
        ...
        # Exclude House bills "placed on calendar" - routine procedure, too noisy
        # Senate "placed on calendar" (especially Rule XIV) is more significant
        if 'placed on' in action_lower and 'calendar' in action_lower:
            house_types = ['hr', 'hres', 'hjres', 'hconres']
            if bill_type_lower in house_types:
                return False
...
def run_tracker(post_to_twitter: bool = True, max_posts: int = 10) -> None:
    ...
    for bill in bills:
        ...
        if not is_significant_action(action_text, bill_type):
...
def create_event_post(event: Dict) -> str:
    ...
    if event_type == 'Confirmation':
        ...
        if title:
            # Split the title by bill patterns and pair each bill with its description
            bill_split_pattern = r'(H\.?R\.?\s*\d+|S\.?\s*\d+|H\.?J\.?\s*Res\.?\s*\d+|S\.?J\.?\s*Res\.?\s*\d+|H\.?\s*Res\.?\s*\d+|S\.?\s*Res\.?\s*\d+)'
            parts = re.split(bill_split_pattern, title, flags=re.IGNORECASE)
            
            # Check if we found any bills
            if len(parts) > 1:
                lines.append("Bills for Markup:")
                lines.append("")
                
                # parts will be like: ['Business meeting to consider ', 'H.R.260', ', to require a strategy...', 'S.2904', ', to impose...']
                i = 1  # Start after any preamble
                first_bill = True
                while i < len(parts):
                    bill_ref = parts[i].strip() if i < len(parts) else ''
                    description = parts[i+1].strip() if i+1 < len(parts) else ''
                    
                    # Clean up the description (remove leading comma/colon)
                    description = re.sub(r'^[,:\s]+', '', description).strip()
                    # Remove trailing comma if present
                    description = re.sub(r',\s*$', '', description).strip()
                    
                    # Normalize bill reference
                    bill_ref_normalized = bill_ref.upper().replace(' ', '').replace('.', '')
                    bill_ref_formatted = bill_ref_normalized
                    if bill_ref_formatted.startswith('HR'):
                        bill_ref_formatted = 'H.R.' + bill_ref_formatted[2:]
                    elif bill_ref_formatted.startswith('HJRES'):
                        bill_ref_formatted = 'H.J.Res.' + bill_ref_formatted[5:]
                    elif bill_ref_formatted.startswith('HRES'):
                        bill_ref_formatted = 'H.Res.' + bill_ref_formatted[4:]
                    elif bill_ref_formatted.startswith('SJRES'):
                        bill_ref_formatted = 'S.J.Res.' + bill_ref_formatted[5:]
                    elif bill_ref_formatted.startswith('SRES'):
                        bill_ref_formatted = 'S.Res.' + bill_ref_formatted[4:]
                    elif bill_ref_formatted.startswith('S') and bill_ref_formatted[1:].isdigit():
                        bill_ref_formatted = 'S.' + bill_ref_formatted[1:]
                    
                    # Add blank row between bills (not before first one)
                    if not first_bill:
                        lines.append("")
                    first_bill = False
                    
                    if bill_ref and description:
                        # Add "A bill " prefix if not already present
                        if not description.lower().startswith('a bill '):
                            description = 'A bill ' + description[0].lower() + description[1:] if description else description
                        else:
                            # Capitalize existing "A bill" properly
                            description = 'A bill ' + description[7:]
                        lines.append(f"{bill_ref_formatted}: {description}")
                    elif bill_ref:
                        lines.append(f"{bill_ref_formatted}")
                    
                    i += 2
            else:
                # No bills detected - use original title
                lines.append("Bills for Markup:")
                lines.append("")
                lines.append(title)
