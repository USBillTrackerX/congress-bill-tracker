...
# Try to import BeautifulSoup for House calendar parsing
try:
    from bs4 import BeautifulSoup
    HAS_BEAUTIFULSOUP = True
except ImportError:
    HAS_BEAUTIFULSOUP = False
    logger = logging.getLogger(__name__)  # Will be properly configured later
...
def fetch_senate_calendar() -> List[Dict]:
    ...
    Returns:
        List of meeting dicts with standardized format
    """
    ...
    try:
        ...
        for meeting in root.findall('meeting'):
            ...
            # Parse date: "27-JAN-2026 03:00 PM"
            meeting_date = None
            meeting_time = ''
            if date_str:
                try:
            ...
            # Determine if this is a markup (business meeting with votes)
            ...
            meeting_type = 'Markup' if is_markup else 'Hearing'
            ...
            meetings.append({
                ...
                'room': room_text,
                'status': 'Scheduled',
                'is_future': meeting_date > datetime.now()
            })
        ...
        try:
            ...
            for row in soup.find_all('tr', class_='meetingRow'):
                try:
                    ...
                    is_markup = (
                        'markup' in title_lower or
                        'business meeting' in title_lower or
                        'to consider' in title_lower
                    )
            ...
            logger.warning(f"Error fetching House calendar for {date_range}: {e}")
    
    return meetings


def fetch_upcoming_markups(days_ahead: int = 14) -> List[Dict]:
    """
    Fetch upcoming committee markups from both chambers.
    
    Args:
        days_ahead: How many days ahead to look
        
    Returns:
        List of markup meetings within the time window
    """
    logger.info("Fetching upcoming markups from official sources...")
    
    # Fetch from both sources
    logger.info("  Fetching Senate calendar...")
    senate_meetings = fetch_senate_calendar()
    logger.info(f"    Senate: {len(senate_meetings)} total meetings")
    
    logger.info("  Fetching House calendar...")
    ...
    for meeting in all_meetings:
        # Only markups
        if meeting.get('type') != 'Markup':
            continue
        
        # Only future meetings within window
    ...
    logger.info(f"  Found {len(markups)} markups in next {days_ahead} days")
    return markups


def create_markup_post(markup: Dict, post_type: str = 'new') -> str:
    """
    Create tweet text for a markup event.
    
    Args:
        markup: Meeting dict
        post_type: 'new', 'reminder', or 'canceled'
        
    Returns:
        Tweet text
    """
    committee = markup.get('committee', 'Unknown Committee')
    ...
    if meeting_time:
        date_time = f"{formatted_date} at {meeting_time}"
    else:
        date_time = formatted_date
    
    # Extract bill references from title
    bills = []
    bill_pattern = r'(H\.?R\.?\s*\d+|S\.?\s*\d+|H\.?\s*Res\.?\s*\d+|S\.?\s*Res\.?\s*\d+)'
    matches = re.findall(bill_pattern, title, re.IGNORECASE)
    ...
    # Build tweet
    if post_type == 'new':
        header = f"📅 MARKUP SCHEDULED | {committee}"
    elif post_type == 'reminder':
        header = f"⏰ MARKUP TOMORROW | {committee}"
    else:
        header = f"❌ MARKUP CANCELED | {committee}"
    
    lines = [header]
    lines.append(f"Date: {date_time}")
    
    # Add what they're voting on
    if title:
        # Truncate if too long
        if len(title) > 200:
            title = title[:197] + "..."
        lines.append(f"Committee will vote on: {title}")
    
    if bills:
        lines.append(f"Bills: {', '.join(bills[:5])}")  # Max 5 bills
    
    lines.append("")
    lines.append("Watch live: congress.gov/committees")
...
def run_tracker(post_to_twitter: bool = True, max_posts: int = 10) -> None:
    ...
    if not candidates:
        ...
        # Don't return - continue to Part 2 for markups
    ...
    for candidate in candidates:
        if action_posts_made >= max_posts:
        ...
        logger.info(f"Posting action (priority {candidate['priority']}): {tweet_text[:100]}...")
    ...
    # Save bill action data
    save_posted_actions(posted_actions)
    save_bill_status(bill_status)
    
    logger.info(f"Part 1 complete. Made {action_posts_made} action posts.")
    
    # =========================================================================
    # PART 2: MARKUP CALENDAR POSTS
    # =========================================================================
    logger.info("="*60)
    ...
    markup_posts_made = 0
    max_markup_posts = 5  # Limit markup posts per run
    
    # Fetch upcoming markups
    upcoming_markups = fetch_upcoming_markups(days_ahead=14)
    
    # Track which markups we've already posted about
    current_markup_ids = set()
    new_markups = []
    
    for markup in upcoming_markups:
        event_id = markup.get('eventId', '')
        if not event_id:
            continue
        
        current_markup_ids.add(event_id)
        
        # Check if we've already posted about this markup
        if event_id not in scheduled_markups:
            new_markups.append(markup)
    
    logger.info(f"Found {len(upcoming_markups)} upcoming markups, {len(new_markups)} are new")
    
    # Post new markups (up to limit)
    for markup in new_markups[:max_markup_posts - markup_posts_made]:
        if markup_posts_made >= max_markup_posts:
            break
        
        event_id = markup.get('eventId', '')
        tweet_text = create_markup_post(markup, post_type='new')
        ...
        if post_to_twitter:
            if post_to_x(x_client, tweet_text):
                scheduled_markups[event_id] = {
                    'posted_at': datetime.now().isoformat(),
                    'committee': markup.get('committee', ''),
                    'date': markup.get('formatted_date', ''),
    ...
    total_posts = action_posts_made + markup_posts_made
    logger.info("="*60)
    logger.info(f"Finished. Made {total_posts} total posts ({action_posts_made} actions, {markup_posts_made} markups)")
    logger.info("="*60)
