...
def build_calendar_cache() -> list:
    ...
    for chamber in ['house', 'senate']:
        ...
        for meeting_item in meetings_list:
            ...
            # Extract bills mentioned in title
            title = details.get('title', '')
            bills_mentioned = parse_bills_from_title(title)
            ...
            all_meetings.append({
                ...
                'is_future': meeting_date >= today_start,
                'details': details  # Keep full details for markup posts
            })
            
            time.sleep(0.02)  # Small delay
    
    logger.info(f"  Cached {len(all_meetings)} meetings total")
...
def get_upcoming_markups_from_cache(days_ahead: int = 14) -> list:
    ...
    for meeting in cache:
        ...
        markups.append(meeting['details'])
