...
# Cache for current president (to avoid repeated API calls in same run)
_current_president_cache = None


def fetch_current_president() -> str:
    """
    Fetch the current US president's name from Wikipedia API.
    
    Returns:
        President's last name (e.g., "Trump", "Biden") or empty string as fallback
    """
    global _current_president_cache
    
    # Return cached value if available
    if _current_president_cache:
        return _current_president_cache
    
    try:
        # Use Wikipedia API to get current president
        url = "https://en.wikipedia.org/api/rest_v1/page/summary/President_of_the_United_States"
        response = requests.get(url, timeout=10)
        response.raise_for_status()
        data = response.json()
        
        # The extract contains info about current president
        extract = data.get('extract', '')
        
        # Parse the president's name from the extract
        # Usually starts with "The president of the United States is [Name]"
        match = re.search(r'president of the United States is ([A-Z][a-z]+ [A-Z][a-z]+)', extract, re.IGNORECASE)
        if match:
            full_name = match.group(1)
            last_name = full_name.split()[-1]
            _current_president_cache = last_name
            logger.info(f"Fetched current president: {last_name}")
            return last_name
        
        # Fallback: try to find "President [Name]" pattern
        match = re.search(r'President ([A-Z][a-z]+ [A-Z][a-z]+)', extract)
        if match:
            full_name = match.group(1)
            last_name = full_name.split()[-1]
            _current_president_cache = last_name
        ...
        logger.error(f"Error fetching current president: {e}")
    
    return ""  # Return empty string, not "President"
...
def create_tweet_text(bill: dict, action: dict, bill_summaries: dict, anthropic_client: anthropic.Anthropic) -> str:
    ...
    url_length = 23
    fixed_parts = f"{line1}\n{line2}\n{line3}\n{line4}\n\n\n[AI-Generated]\n\n{' ' * url_length}"
    available_for_summary = 280 - len(fixed_parts) - len("Summary: ")
