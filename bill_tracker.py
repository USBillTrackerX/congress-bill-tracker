...
def get_committee_info(bill: dict) -> str:
    ...
    if committee_names:
        return ', '.join(committee_names)  # Use comma instead of semicolon for | separator format
...
def create_tweet_text(bill: dict, action: dict, bill_summaries: dict, anthropic_client: anthropic.Anthropic) -> str:
    ...
    # Get what's next explanation
    whats_next = get_whats_next(action_text)
    ...
    # Build the tweet
    # Line 1: Bill ID | Title | Sponsor | Committee(s)
