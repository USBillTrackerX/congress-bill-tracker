...
2. Do NOT repeat the bill name twice - start with "The [Name]" only once, then continue with the verb
...
Respond with ONLY the summary starting with "The [Bill Name]..." followed immediately by a verb, no other preamble or labels."""
    else:
...
2. Do NOT repeat the bill name twice - start with "The [Name]" only once, then continue with "would", "aims to", etc.
...
def run_tracker(post_to_twitter: bool = True, max_posts: int = 10) -> None:
    ...
    Optimized approach:
    1. Fetch bill list (basic info only) - fast
    2. For each bill, fetch just actions (lightweight)
    3. Filter significant actions, check if already posted, assign priority
    4. Sort by priority
    5. Only fetch full bill details for the top candidates
    6. Post up to max_posts
    """
    ...
    # Fetch recent bills (basic info only)
    bills = fetch_recent_bills(days_back=1)
    
    # PHASE 1: Quickly collect candidate actions (no full bill details yet)
    logger.info("Phase 1: Scanning for significant actions...")
    candidates = []
    ...
    for bill in bills:
        ...
        # Fetch only actions (lightweight call)
        ...
        for action in actions:
            ...
            # Add candidate with priority (no full details yet)
        ...
        # Early exit: if we have enough high-priority candidates, stop scanning
        high_priority_count = sum(1 for c in candidates if c['priority'] <= 2)
        if high_priority_count >= max_posts * 2:
            logger.info(f"Found enough high-priority candidates ({high_priority_count}), stopping scan early")
            break
    
    logger.info(f"Phase 1 complete: Found {len(candidates)} candidate actions")
    
    if not candidates:
        logger.info("No new actions to post")
    ...
    # PHASE 2: Fetch full details only for top candidates and post
    logger.info("Phase 2: Fetching details and posting...")
    posts_made = 0
    ...
    for candidate in candidates:
        ...
        # NOW fetch full bill details (only for bills we're actually posting)
        bill_details = fetch_bill_details(bill_type, bill_number)
        if not bill_details:
            logger.warning(f"Could not fetch details for {bill_type}{bill_number}, skipping")
            continue
